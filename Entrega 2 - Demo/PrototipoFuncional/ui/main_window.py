...
class MainWindow:
    ...
    def create_input_section(self, parent):
        ...
        input_frame = ttk.LabelFrame(parent, text="Entrada de Datos", padding="5")
        ...
        ttk.Button(input_frame, text="Cargar Archivo", 
                  command=self.load_file).grid(row=1, column=0, padx=(0, 5))
        ttk.Button(input_frame, text="Limpiar", 
                  command=self.clear_text).grid(row=1, column=1, padx=5)
        ttk.Button(input_frame, text="Texto de Ejemplo", 
                  command=self.load_sample_text).grid(row=1, column=2, padx=(5, 0))
    ...
    def create_controls_section(self, parent):
        ...
        ttk.Button(controls_frame, text="Procesar con Huffman", 
                  command=self.process_huffman).grid(row=0, column=0, padx=(0, 5))
        ttk.Button(controls_frame, text="Procesar con Shannon-Fano", 
                  command=self.process_shannon_fano).grid(row=0, column=1, padx=5)
        ttk.Button(controls_frame, text="Comparar Algoritmos", 
                  command=self.compare_algorithms).grid(row=0, column=2, padx=5)
        ttk.Button(controls_frame, text="Exportar PDF", 
                  command=self.export_pdf).grid(row=0, column=3, padx=(5, 0))
    ...
    def create_tables_tab(self):
        ...
        self.notebook.add(tables_frame, text="Tablas de Códigos")
    ...
    def load_sample_text(self):
        ...
        sample_text = """La teoría de la información es una rama de las matemáticas aplicadas que estudia la cuantificación, almacenamiento y comunicación de información. Fue desarrollada por Claude Shannon en 1948. Los algoritmos de compresión como Huffman y Shannon-Fano son fundamentales en esta área."""
        self.text_area.delete(1.0, tk.END)
